...
class Auth
{
	...
	public function login($login, $password, $remember=FALSE)
	{
		...
		$user = $this->ci->user_model->get_user_by_credentials($login,do_hash($password));
		...
		$this->user = $user;
		$this->ci->session->set_userdata('connected_user',$this->user);
	...
	public function user()
	{
		...
		$session_user = $this->ci->session->userdata('connected_user');
		
		if($session_user!= null && !empty($session_user)) {
	 ...
	 * @access public
	 *	
	 * @message string required. (Optional) Permission to display upon restricting. To indicate the reason why 
